using LearnGeneric.Helpers;
using LearnGeneric.Products;
	...
	public class ProductManager
	{
		...
		public void Init()
		{
			...
			while (isContinue)
			{
				...
				var option = ValidateHelper.ValidateInt("option", s => s <= 5 && s >= 1);
				switch (option)
				{
					case 1:
						HandleAddProduct();
		...
		private void HandleAddProduct()
		{
			...
			product.Price = ValidateHelper.ValidateDecimal("product price", s => s > 0);

			product.Inventory = ValidateHelper.ValidateInt("product inventory", s => s > 0);
